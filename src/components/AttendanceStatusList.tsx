  ...
  const fetchAttendanceStatus = async () => {
    ...
    try {
      ...
      const { data: attendanceData, error: attendanceError } = await supabase
        ...
        .select('staff_uid, check_in_time, check_out_time, status')
        .eq('date', today)
      ...
      const attendanceMap = new Map<string, { checkInTime: string | null; checkOutTime: string | null; status: string | null }>();
                      ...
                      {checkedInStaff.map(staff => (
                        ...
                        >
                          <div className="flex items-center gap-2 min-w-0">
                            <span className="font-medium truncate max-w-[120px]">{staff.name}</span>
                            {staff.status && (
                              <Badge className={`text-[10px] px-1.5 py-0 ${getStatusColor(staff.status)}`}>
