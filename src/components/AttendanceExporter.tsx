    ...
    try {
      ...
      attendanceData.forEach((record: any, index: number) => {
        ...
        const row = worksheet.addRow({
          ...
          jenisAbsensi: record.attendance_type === 'overtime' ? 'LEMBUR' : (record.attendance_type === 'shift' ? 'SHIFT' : ''),
          onDuty: onDuty,
          offDuty: offDuty,
        ...
        } else if (record.device_flag || record.device_flag_in || record.device_flag_out) {
          row.eachCell((cell) => {
  ...
  const getExportData = async () => {
    ...
    return attendanceData.map((record: any, index: number) => {
      ...
      return {
        ...
        attendanceType: isEmpty ? '' : (record.attendance_type === 'overtime' ? 'LEMBUR' : (record.attendance_type === 'shift' ? 'SHIFT' : '')),
        onDuty: onDuty,
        offDuty: offDuty,
